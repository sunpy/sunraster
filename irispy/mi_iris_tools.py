...
def get_iris_response(pre_launch=False, response_file=None, response_version=None,
    ...
    if iris_response["VERSION"] > 2:
        ...
        c_f_time = iris_response["C_F_TIME"]
        ...
        c_n_time = iris_response["C_N_TIME"]
        ...
        c_s_time = iris_response["C_S_TIME"]
...
def fit_iris_xput(time_obs, time_cal_coeffs, cal_coeffs):
    ...
    if size_time_cal_coeffs[1] != 2 or size_cal_coeffs[1]  < 2:
       ...
       return
        ...
        t_cal_coeffs = np.float64(time_cal_coeffs)
        t_ref = parse_time('1979-01-01', time=1)

        count = 0
        for i in time_obs:
            aux_cal_coeffs = np.zeros(2*size_time_cal_coeffs[0])
            t_obs = parse_time(i, utime =1) - t_ref
            t_obs = t_obs.total_seconds()
        
            # Looking for the closest time in the calibration time intervals.
            # Differences are given in years
            t_diff = t_obs - t_cal_coeffs
            t_diff = t_diff/yr2sec
            t_diff = t_diff.reshape(size_time_cal_coeffs[0]*size_time_cal_coeffs[1])
            w = (np.where((t_diff - np.abs(t_diff)) !=0))
            w = w[0][0]
            ...
            out[count] = np.matmul(aux_cal_coeffs, cal_coeffs[:,:2].reshape((aux_cal_coeffs.shape[0])))
            count+=1
