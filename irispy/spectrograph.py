    ...
    * `IRIS FITS Header keywords <https://www.lmsal.com/iris_science/doc?cmd=dcur&proj_num=IS0077&file_type=pdf>`_
    ...
    def __repr__(self):
        ...
        spectral_windows_info = "".join(
            [
                "\n{0:>15} : {1:20} pix".format(
                    name, str([int(dim.value) for dim in self.data[name].dimensions])
                )
        ...
        return (
            ...
            obsid=self.meta["OBSID"],
            obsdesc=self.meta["OBS_DESC"],
            obs_period=obs_period,
            inst_period=instance_period,
            nraster=self.meta["NRASTERP"],
    ...
    def spectral_windows(self):
        ...
        for key in list(self.data.keys()):
            if type(self.data[key]) == IRISSpectrogramCubeSequence:
                spectral_window_list.append(
                    [self.data[key].meta[colname] for colname in colnames]
                )
...
class IRISSpectrogramCubeSequence(NDCubeSequence):
    ...
    def __init__(self, data_list, meta=None, common_axis=0):
        ...
        if not all([key in list(meta) for key in required_meta_keys]):
            raise ValueError(
                "Meta must contain following keys: {0}".format(required_meta_keys)
            )
...
""".format(
            obs_repr=_produce_obs_repr_string(self.data[0].meta),
...
class IRISSpectrogramCube(NDCube):
    ...
    def __init__(
        ...
        if not all([key in extra_coords_keys for key in required_extra_coords_keys]):
            raise ValueError(
                "The following extra coords must be supplied: {0} vs. {1} from {2}".format(
...
Axis Types: {axis_types}
""".format(
            obs_repr=_produce_obs_repr_string(self.meta),
        ...
        if f == 0:
            ...
            for i, window_name in enumerate(spectral_windows_req):
                ...
                window_metas[window_name] = {
                    "detector type": hdulist[0].header[
                        "TDET{0}".format(window_fits_indices[i])
                    ],
                    "spectral window": hdulist[0].header[
                        "TDESC{0}".format(window_fits_indices[i])
                    ],
                    "brightest wavelength": hdulist[0].header[
                        "TWAVE{0}".format(window_fits_indices[i])
                    ],
                    "min wavelength": hdulist[0].header[
                        "TWMIN{0}".format(window_fits_indices[i])
                    ],
                    "max wavelength": hdulist[0].header[
                        "TWMAX{0}".format(window_fits_indices[i])
                    ],
    ...
    data = dict(
        [
            (
                window_name,
                IRISSpectrogramCubeSequence(
                    data_dict[window_name], window_metas[window_name], common_axis=0
                ),
            )
