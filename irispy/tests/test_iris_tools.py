...
response_url = (
    "https://sohowww.nascom.nasa.gov/solarsoft/iris/response/iris_sra_c_20161022.geny"
)
...
def test_convert_between_DN_and_photons(
    ...
    output_arrays, output_unit = iris_tools.convert_between_DN_and_photons(
...
def test_calculate_exposure_time_correction(
    input_arrays, old_unit, exposure_time, force, expected_arrays, expected_unit
):
...
def test_calculate_exposure_time_correction_error(
    ...
    assert pytest.raises(
        ValueError,
        iris_tools.calculate_exposure_time_correction,
        input_arrays,
        old_unit,
        exposure_time,
        force,
...
def test_uncalculate_exposure_time_correction(
    input_arrays, old_unit, exposure_time, force, expected_arrays, expected_unit
):
...
def test_uncalculate_exposure_time_correction_error(
    ...
    with pytest.raises(ValueError):
        assert iris_tools.uncalculate_exposure_time_correction(
            input_arrays, old_unit, exposure_time, force=force
        )
...
def test_get_iris_response_response_file():
    assert pytest.raises(
        KeyError, iris_tools.get_iris_response, time_obs, response_file="hello.py"
    )
...
iris_response1 = get_iris_response(
...
iris_response2 = get_iris_response(
...
iris_response3 = get_iris_response(
...
iris_response4 = get_iris_response(
...
def test_calculate_dust_mask(input_array, expected_array):
    np_test.assert_array_equal(
        iris_tools.calculate_dust_mask(input_array), expected_array
    )
...
def test_fit_iris_xput(input_arrays, expected_array):
    np_test.assert_almost_equal(
        fit_iris_xput(input_arrays[0], input_arrays[1], input_arrays[2]),
        expected_array,
        decimal=6,
    )
