...
from irispy.spectrograph import (IRISSpectrogramCube, IRISSpectrogramCubeSequence,
                                 IRISSpectrograph, read_iris_spectrograph_level2_fits)
...
extra_coords0 = [
    (
        ...
        np.array(
            [
                datetime.datetime(2017, 1, 1) + datetime.timedelta(seconds=i)
...
@pytest.mark.parametrize(
    "input_cube, new_unit, expected_cube",
    [
...
def test_IRISSpectrogramCube_apply_exposure_time_correction(
    input_cube, undo, force, expected_cube
):
...
@pytest.mark.parametrize(
    "input_sequence, new_unit, expected_sequence",
    [
...
def test_IRISSpectrogramCubeSequence_convert_to(
    input_sequence, new_unit, expected_sequence
):
