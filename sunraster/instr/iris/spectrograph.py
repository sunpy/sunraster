...
class IRISSpectrograph(object):
    ...
    Parameters
    ----------
    data: `dict` of `sunraster.instr.iris.spectrograph.IRISSpectrogramCubeSequence`.
    ...
    * `IRIS FITS Header keywords <https://www.lmsal.com/iris_science/doc?cmd=dcur&proj_num=IS0077&file_type=pdf>`_
    ...
    def spectral_windows(self):
        ...
        for key in list(self.data.keys()):
            if isinstance(self.data[key], IRISSpectrogramCubeSequence):
...
class IRISSpectrogramCubeSequence(RasterSequence):
    ...
    def __init__(self, data_list, meta=None, slit_step_axis=0):
        ...
        # Initialize Sequence.
        super().__init__(data_list, meta=meta, slit_step_axis=slit_step_axis)
...
""".format(obs_repr=_produce_obs_repr_string(self.data[0].meta),
    ...
    def convert_to(self, new_unit_type, copy=False):
        ...
        if copy is True:
            return IRISSpectrogramCubeSequence(
                converted_data_list, meta=self.meta, slit_step_axis=self._slit_step_axis)
        else:
            self.data = converted_data_list


class IRISSpectrogramCube(Raster):
    """
